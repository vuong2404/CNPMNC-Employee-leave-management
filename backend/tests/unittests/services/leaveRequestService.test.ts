...
>;
import { LeaveRequestStatus, DEFAULT_LEAVE_DAYS } from "../../../src/Constants/common";
...
describe("leaveRequestService", () => {
	...
	describe("approve", () => {
		it("should throw ForbiddenError if action request invalid", async () => {
			...
			const mockLeaveDays: LeaveDay[] = [
				new LeaveDay({ date: moment("12-23-2023", "MM-DD-YYYY").toDate() }),
				new LeaveDay({ date: moment("2-27-2024", "MM-DD-YYYY").toDate() }),
			];
	...
	describe("deleteRequest", () => {
		it("should delete leave request by ID", async () => {
			...
			expect(mockLeaveRequestRepository.deleteLeaveRequest).toHaveBeenCalledWith(
				1,
				2,
			);
			expect(mockResponse.send).toHaveBeenCalledWith({
		...
		it("should call next with an error if an exception occurs", async () => {
			...
			mockLeaveRequestRepository.deleteLeaveRequest.mockRejectedValue(
